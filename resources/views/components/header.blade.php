...
>
    <div class="container px-4 pt-4 sm:px-6 sm:pt-5">
        <div
            class="grid grid-cols-[auto_1fr_auto] items-center gap-3 border border-white/8 bg-black px-3 py-3 sm:gap-4 sm:px-4 md:px-5 lg:gap-6 lg:px-6 lg:py-4"
        >
            <a href="{{ route('home') }}" class="flex items-center gap-3 sm:gap-4" aria-label="ARIES Investissements — accueil">
                <img
                ...
                >
                <span class="hidden xl:block text-[11px] uppercase tracking-[0.24em] text-white/50">
                    Conseil financier & investissement
                </span>
            </a>

            <nav class="hidden lg:flex items-center justify-center gap-6 xl:gap-8 px-6 xl:px-8" aria-label="Navigation principale">
                @foreach ($links as $l)
                    @php $active = request()->routeIs($l['route']); @endphp
                    <a
                        href="{{ route($l['route']) }}"
                        class="group relative whitespace-nowrap text-[12px] uppercase tracking-[0.18em] {{ $active ? 'text-white' : 'text-white/76' }} transition-colors hover:text-white"
                    >
                        {{ $l['label'] }}
            ...
            <div class="hidden lg:flex items-center gap-5">
                <div class="text-right">
                    <p class="text-[11px] uppercase tracking-[0.22em] text-white/42">Brazzaville · Washington</p>
                    <p class="mt-1 text-xs text-white/65">Afrique · Conseil stratégique · Banque d’affaires</p>
                </div>
            </div>

            <button
                @click="open = !open"
                class="justify-self-end border border-white/10 p-2.5 text-white transition hover:bg-white/5 lg:hidden"
                :aria-expanded="open"
        ...
        <div
            ...
            class="mt-2 border border-white/8 bg-black lg:hidden"
        >
            <nav class="flex flex-col gap-1 p-4 sm:p-5">
                @foreach ($links as $l)
                    ...
                    <a
                        href="{{ route($l['route']) }}"
                        class="border-b border-white/8 px-0 py-3.5 text-base sm:text-lg {{ $active ? 'text-gold' : 'text-white' }}"
                    >
                        {{ $l['label'] }}
                    </a>
                @endforeach
